...
class RWP:
    def __init__(self, model, proxy_1, proxy_2, proxy_2_optim, config):
        super(RWP, self).__init__()
        self.model = model
        self.proxy_1 = proxy_1
        self.proxy_2 = proxy_2
        self.proxy_2_optim = proxy_2_optim
        self.gamma = config.rwp["gamma"]
        self.warmup = config.rwp["warmup"]

    def calc_awp(self, inputs_adv, targets, lsc_loss=1.7, iterations=10):
        diff = OrderedDict()
        diff_scale = OrderedDict()
        ...
        for ii in range(iterations):
            ...
            loss = nn.CrossEntropyLoss(reduction="none")(output, targets)

            device = loss.device
            indicator = (loss < lsc_loss).to(device).type(torch.float)
